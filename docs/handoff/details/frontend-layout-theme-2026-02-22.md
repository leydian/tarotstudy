...
## 9. 기하학적 스프레드 시각화 (2026-02-23)
- **배열판 스타일**: 그리드 시스템을 이용해 V, 다이아몬드, 십자 등 실제 타로 배열 모양을 직관적으로 구현.
*   **텍스처 패턴**: Radial-gradient와 도트 패턴 배경을 적용하여 실제 상담 데스크와 같은 입체감 제공.
- **모바일 리스트 뷰**: 모바일 환경에서 복잡한 그리드 대신 수직 리스트로 확인 가능한 토글 모드 지원.

## 11. UI 정규화 및 클린업 (2026-02-24)
- **인라인 스타일 완전 제거**: 컴포넌트 내 `style` 속성을 `features/chat/entry.css` 및 `features/spreads/entry.css` 클래스로 대체하여 스타일 소스 일원화.
- **메신저형 대화 UI**: 타로리더(✦)와 학습리더(🛡️)를 시각적으로 분리한 전용 말풍선 디자인 적용.
- **초소형 기기 최적화**: 480px 이하 기기에서 패딩, 폰트 크기, 카드 너비를 자동 조정하는 미디어 쿼리 도입.
- **카드 스포트라이트**: 채팅 내 카드 크기를 140px로 최적화하여 텍스트와의 시각적 밸런스 유지.
- **접근성(ARIA)**: 주요 버튼 및 입력창에 스크린 리더용 라벨 보강.
