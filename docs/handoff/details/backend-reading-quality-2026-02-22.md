...
## 12. 개인화 복기 피드백 루프 (2026-02-23)
- **데이터 연동**: `performSpreadDraw`에서 `userId`를 통해 `progress-store`의 과거 복기 데이터를 조회함.
- **지능형 브릿지**: 리딩 생성 시 사용자가 과거에 동일 카드를 뽑고 남긴 리뷰가 있다면, 리드 문구에 "(참고: 지난번 이 카드는 'OOO'로 기록하셨네요)" 멘트를 주입함.
- **효과**: 단순 일회성 리딩에서 나아가 사용자의 고유한 데이터가 누적되고 반영되는 개인화 경험(Personalized Reading) 제공.
