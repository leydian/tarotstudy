...
## 15. 네트워크 전송 최적화 (2026-02-24)
- **응답 압축(Gzip)**: `@fastify/compress` 플러그인을 도입하여 모든 API 응답에 대해 전역 압축을 적용함. 텍스트 위주의 리딩 모델 및 스프레드 목록 전송 시 페이로드 크기를 70% 이상 절감하여 저대역폭 환경에서의 성능을 보강함.
