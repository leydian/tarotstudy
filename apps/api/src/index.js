...
function buildSpreadCatalog(baseSpreads, targetCount = 100) {
...
function buildImmersiveEvidenceLine({ spreadId = '', position = '', card = '', orientation = '정방향', keyword = '핵심', index = 0, intent = 'general', context = '' }) {
  ...
  if (spreadId === 'monthly-fortune' && /월간 테마/.test(position)) {
    return `${position}의 ${card} ${orientationKo} 카드는 이번 달 전체 운영 기준을 먼저 고정하라는 신호입니다.`;
  }
  ...
  if (spreadId === 'relationship-recovery') {
    if (/현재 관계|상태/.test(position)) return `${position}의 ${card} ${orientationKo} 카드는 관계의 현재 온도를 사실 기준으로 확인하라는 뜻입니다.`;
    if (/문제/.test(position)) return `${position}의 ${card} ${orientationKo} 카드는 충돌을 키우는 트리거를 먼저 분리해야 한다는 신호입니다.`;
    if (/해결/.test(position)) return `${position}의 ${card} ${orientationKo} 카드는 길게 설명하기보다 짧은 확인 문장이 더 유효하다는 기준을 제시합니다.`;
    if (/상대/.test(position)) return `${position}의 ${card} ${orientationKo} 카드는 추측보다 반응 단서를 관찰해야 한다는 신호입니다.`;
    if (/행동/.test(position)) return `${position}의 ${card} ${orientationKo} 카드는 오늘 실행할 행동 1개로 관계 리듬을 복구하라는 메시지입니다.`;
    if (/7일/.test(position)) return `${position}의 ${card} ${orientationKo} 카드는 다음 7일 동안 대화 강도를 조절해야 재충돌을 줄일 수 있다는 의미입니다.`;
  }
  ...
  if (spreadId === 'celtic-cross') {
    if (/현재|도전/.test(position)) return `${position}의 ${card} ${orientationKo} 카드는 현재 변수와 저항 지점을 동시에 보라는 신호입니다.`;
...
function buildCautionV3({ context = '', spreadId = '', domain = 'general', verdictLabel = '조건부', questionType = 'open' }) {
  const seed = buildSeed(`${spreadId}:${context}:${domain}:${verdictLabel}:${questionType}`);
  const pools = {
    study: {
  ...
  return softenAbsolutes(pickBySeed(domainPool[key] || pools.general[key], seed));
}
...
function pickBySeed(pool = [], seed = 0) {
...
function normalizeEverydaySummaryTone(raw = '') {
  let text = String(raw || '');
  const replacements = [
    [/실행축/g, '실행 흐름'],
    [/결과축/g, '결과 흐름'],
...
function buildSummaryTheme({ spreadName, context = '', items = [], topKeywords = [] }) {
  ...
  if (spreadName === '일별 운세') {
    return `한 줄 테마: 오늘은 '${leadKeyword}' 흐름을 보고 우선순위를 좁히면 더 안정적으로 움직일 수 있습니다.`;
  }
  if (spreadName === '주별 운세') {
    return `한 줄 테마: 이번 주는 '${leadKeyword}'을 중심으로, 잘되는 날은 밀고 힘든 날은 쉬어가면 좋습니다.`;
  }
  if (spreadName === '월별 운세') {
    return `한 줄 테마: 이번 달은 '${leadKeyword}' 기준으로 확장할 때와 조절할 때를 나누면 흐름이 더 선명해집니다.`;
  }
  if (spreadName === '연간 운세 (12개월)') {
    return `한 줄 테마: 올해는 '${leadKeyword}'을 기준으로 분기마다 속도를 나누면 흔들림을 줄이기 좋습니다.`;
  }
  if (spreadName === '양자택일 (A/B)') {
    const choiceSnapshot = buildChoiceABSnapshot({ items, context });
    if (choiceSnapshot.mode === 'single') {
      return `한 줄 테마: '${leadKeyword}' 흐름을 보면서 관계 속도를 낮추고, 오래 갈 수 있는지 먼저 확인하는 게 핵심입니다.`;
    }
    if (choiceSnapshot.recommendationLine.includes('A 쪽이')) {
      return `한 줄 테마: '${leadKeyword}' 기준으로 보면 A쪽이 조금 우세하니, 당장 반응보다 3개월 유지 가능성을 먼저 비교하세요.`;
    }
    if (choiceSnapshot.recommendationLine.includes('B 쪽이')) {
      return `한 줄 테마: '${leadKeyword}' 기준으로 보면 B쪽이 조금 우세하니, 지금 만족보다 오래 갈 때의 소모를 같이 보세요.`;
    }
    return `한 줄 테마: '${leadKeyword}' 기준으로 A/B가 박빙이라, 같은 체크리스트로 비교해 최종 결정을 좁히는 것이 핵심입니다.`;
  }
  if (spreadName === '3카드 스프레드') {
    return `한 줄 테마: 세 장을 '${leadKeyword}' 한 흐름으로 이어 읽으면, 지금 할 일이 더 또렷해집니다.`;
  }
  if (spreadName === '켈틱 크로스') {
    return `한 줄 테마: 켈틱 리딩은 '${leadKeyword}' 흐름을 시작-걸림-결과로 이어 보면 이해가 더 쉬워집니다.`;
  }
  if (intent === 'relationship' || intent === 'relationship-repair') {
    return `한 줄 테마: 지금은 '${leadKeyword}' 흐름을 보면서 대화 속도와 감정 강도를 함께 조절하는 게 핵심입니다.`;
  }
  if (intent === 'finance') {
    return `한 줄 테마: 지금은 '${leadKeyword}' 흐름을 보고 확장보다 손실 줄이기를 먼저 두는 편이 좋습니다.`;
  }
  if (intent === 'social') {
    return `한 줄 테마: 지금은 '${leadKeyword}' 흐름에 맞춰 태도를 일관되게 가져가면 인상이 안정됩니다.`;
  }
  return `한 줄 테마: 이번 리딩은 '${leadKeyword}' 흐름을 행동 기준 하나로 잡을 때 가장 잘 활용됩니다.`;
}
