...
import type { ReviewChecklist } from '../../pages/spreads-helpers';
import {
  NaturalFlowView,
  UnifiedSummaryView,
        ...
        <article className="panel">
          <div className="spread-page-header">
            ...
            <div className="spread-header-actions">
              ...
              {shouldShowExportActions(Boolean(activeDraw)) && activeDraw && (
                <>
                  <button className="btn" onClick={() => exportReadingTxt(activeDraw, '카드뷰 모드')}>TXT 내보내기</button>
                  <button className="btn" onClick={() => exportReadingPdf(activeDraw, '카드뷰 모드')}>PDF 내보내기</button>
                </>
              )}
            </div>
          </div>
          ...
          {shouldShowPromptBank(Boolean(activeDraw)) && (
                  ...
                  <article key={`mobile-list-${pos.name}`} className="result-item spread-list-item">
                    <div className="spread-list-item-content">
                      ...
                      <div className="spread-list-info">
                        ...
                        {drawn ? (
                          <p className="spread-list-drawn-card">{drawn.card.nameKo} · {drawn.orientation === 'reversed' ? '역방향' : '정방향'}</p>
                        ) : (
                      ...
                      {drawn && (
                        <Link to={`/cards/${drawn.card.id}`}>
                          <TarotImage src={drawn.card.imageUrl} sources={drawn.card.imageSources} cardId={drawn.card.id} alt={drawn.card.nameKo} className="spread-slot-thumb spread-list-thumb-small" />
                        </Link>
                      )}
                    </div>
                  </article>
          ...
          {shouldShowExportActions(Boolean(activeDraw)) && (
            <div className="chip-wrap view-toggle">
              <button className={`chip-link ${detailView === 'reading' ? 'chip-on' : ''}`} onClick={() => setDetailView('reading')}>스프레드 리딩 결과</button>
              <button className={`chip-link ${detailView === 'guide' ? 'chip-on' : ''}`} onClick={() => setDetailView('guide')}>스프레드 가이드/복기</button>
            </div>
          )}

          {detailView === 'reading' && !activeDraw && <p className="sub">카드를 뽑으면 이 영역에 리딩 결과가 표시됩니다.</p>}

          {detailView === 'reading' && activeDraw && (
            <>
              <h4>리딩 메시지</h4>
              {(() => {
                const insights = buildReadingInsights({ spreadId: selected.id, context: activeDraw.context, items: activeDraw.items });
                return (
                  <article className="result-item reading-insights">
                    <div className="verdict-row">
                      <span className={`verdict-badge verdict-${insights.verdict}`}>{insights.verdictLabel}</span>
                      <p className="sub">{renderHighlightedText(normalizeDisplayText(insights.verdictReason), emphasisKeywords, 'verdict-reason')}</p>
                    </div>
                    {insights.conflictWarning && <p className="conflict-warning">{renderHighlightedText(normalizeDisplayText(insights.conflictWarning), emphasisKeywords, 'conflict-warning')}</p>}
              ...
              <div className="reading-dual-panel">
                <article className="result-item reading-summary">
                  ...
                  {toCanonicalReadingLines(activeDraw).length > 0 ? (
                    <NaturalFlowView blocks={toCanonicalReadingLines(activeDraw)} keywords={emphasisKeywords} keyBase="reading-v3" />
                  ) : (
                    <UnifiedSummaryView spreadId={selected.id} summary={activeDraw.summary} keywords={emphasisKeywords} />
                  )}
                </article>
                <article className="result-item reading-coach">
                  <h5 className="reading-block-title">종합 학습 내역</h5>
                  <NaturalFlowView blocks={buildLearningDigest(activeDraw.items).map((line) => splitDigestLine(line).body || line)} keywords={emphasisKeywords} keyBase="learning-digest" compact />
                </article>
              </div>
                ...
                {activeDraw.items.map((item) => (
                  <article key={`${item.position.name}-${item.card.id}`} className="result-item spread-reading-item">
                    <div className="tarot-row">
                      <TarotImage src={item.card.imageUrl} sources={item.card.imageSources} cardId={item.card.id} alt={item.card.nameKo} className={`tarot-thumb ${item.orientation === 'reversed' ? 'card-reversed' : ''}`} loading="lazy" />
                      <div>
                        ...
                        <div className="evidence-chips">
                          <span className="evidence-chip">근거: {item.card.keywords[0]}</span>
                    ...
                    <div className="reading-columns">
                      <div className="reader-block">
                        ...
                        <NaturalFlowView blocks={toParagraphBlocks(mergeTarotMessage(item.coreMessage, item.interpretation))} keywords={item.card.keywords || []} keyBase={`tarot-${item.position.name}`} compact />
                      </div>
                      <div className="learning-point">
                        <h5 className="reading-block-title">학습 코치 요약</h5>
                        <CoachSummaryView lines={toCoachBlocks(item.learningPoint || '카드 근거와 행동을 복기하세요.')} context={{ positionName: item.position.name, cardName: item.card.nameKo, orientation: item.orientation, keywords: item.card.keywords || [], questionContext: activeDraw.context || '', tarotNarrative: mergeTarotMessage(item.coreMessage, item.interpretation) }} />
                      </div>
                    </div>
                  </article>
                ))}
          ...
          {detailView === 'guide' && (
            <>
              <h4>언제 쓰면 좋은가</h4>
              <ul className="clean-list compact-list">{selected.whenToUse.map((item) => <li key={item}>{item}</li>)}</ul>
              <h4>포지션 의미</h4>
              <div className="position-grid">{positions.map((pos) => <article key={pos.name} className="position-card"><p><strong>{pos.name}</strong></p><p>{pos.meaning}</p></article>)}</div>
              <div className="history-header">
                <h4>리딩 복기 기록</h4>
                <button className="btn danger" onClick={() => removeSpreadReadingsBySpreadId(selected.id)}>기록 전체 삭제</button>
              </div>
              <div className="stack">
                {filteredSpreadHistory.map((record) => (
                  <article key={record.id} className="result-item">
                    <p><strong>{new Date(record.drawnAt).toLocaleString()}</strong> · {record.spreadName}</p>
                    <p className="history-summary">{summarizeText(record.summary, 260)}</p>
                    <div className="filters">
                      <select value={record.outcome ?? ''} onChange={(e) => reviewSpreadReading(record.id, e.target.value as any)}>
                        <option value="">결과 체감 선택</option>
                        <option value="matched">맞음</option>
                        <option value="partial">부분적</option>
                        <option value="different">다름</option>
                      </select>
                      <button className="btn" onClick={() => {}}>복기 저장</button>
                    </div>
                  </article>
                ))}
              </div>
...
function CoachSummaryView({ lines, context }: { lines: string[]; context: CoachSummaryContext; }) {
...
function splitCoachLineForDisplay(text: string) {
  ...
  const firstPass = normalized.split(/(?<=[.!?])\s+|(?<=:)\s+| · /g).flatMap((piece) => piece.split(/,\s+| 그리고 | 하지만 | 다만 | 또한 | 및 /g)).map((p) => p.trim()).filter(Boolean);
  const chunks: string[] = [];
  let current = '';
  for (const piece of firstPass) {
    const next = current ? `${current} ${piece}` : piece;
    if (next.length > 44 && current) { chunks.push(current); current = piece; continue; }
    current = next;
  }
...
function groupCoachSummary(lines: string[], context: CoachSummaryContext): CoachSummarySections {
  ...
  const sections: CoachSummarySections = { core: [], action: [], caution: [], question: [] };
  for (const line of cleaned) {
    const split = splitQuestionSegment(line);
    if (split.main) {
      if (/주의|리스크|경고|피할|소모|병목|멈추|조절|금지|불안정/.test(split.main)) sections.caution.push(split.main);
      else if (/행동|실행|시도|적어|기록|루틴|오늘|이번 주|다음/.test(split.main)) sections.action.push(split.main);
      else sections.core.push(split.main);
    }
    if (split.question) sections.question.push(split.question);
  }
...
function splitQuestionSegment(line: string) {
  ...
  return { main: match[1].trim(), question: match[3] ? `복기 질문: ${match[3].trim()}` : '' };
}

function uniqueCoachLines(lines: string[], blacklist: string[]) {
  const seen = new Set<string>();
  const blocked = new Set(blacklist.map((l) => l.toLowerCase()));
...
function buildCoreSignalGuidance(context: CoachSummaryContext) {
  ...
  return `핵심 신호를 "${keyword}"에 고정하세요. ${context.positionName}에서 ${context.cardName}의 의미를 이 키워드 중심으로 좁혀 읽습니다.`;
}

function buildCoachNarrativeParagraphs(sections: CoachSummarySections, context: CoachSummaryContext) {
  return [
    `이번 리딩에서 가장 중요한 포인트는 ${sections.core[0] || '핵심 신호를 고정하는 것'}입니다.`,
    `실행은 ${sections.action[0] || '오늘 바로 실행할 수 있는 작은 단위를 정하세요.'} 구체적으로는 "${context.keywords[0]}"을 기준으로 삼으세요.`,
    `주의할 점은 ${sections.caution[0] || '해석을 넓히기보다 현재의 사실에 집중하세요.'}`,
    `복기는 ${sections.question[0] || '오늘의 실행 결과를 한 문장으로 남기세요.'}`
  ];
}
